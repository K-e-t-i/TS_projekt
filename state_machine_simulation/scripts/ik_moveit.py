...
def main(pose, hold_object):
    ...
    print(pose)
    ref_pose = pq_to_pose(pose[0],pose[1])
    ...
    while not rospy.is_shutdown():
        ...
        iter_no = iter_no + 1
        if hold_object == True:
            pos, orient = gazebo_manager.get_transform('/world', '/ee_link')
            pose_msg = Pose()
            pose_msg.position.x = pos[0]
            pose_msg.position.y = pos[1]
            pose_msg.position.z = pos[2]
            pose_msg.orientation.x = orient[0]
            pose_msg.orientation.y = orient[1]
            pose_msg.orientation.z = orient[2]
            pose_msg.orientation.w = orient[3]
            gazebo_manager.set_model_state_client('object', pose_msg.position, pose_msg.orientation)
        if iter_no>max_iter:
            break
pose = ([0.256, 0.294, 0.365], [0.707, 0.707, 0.0, 0.0])
main(pose,False)
